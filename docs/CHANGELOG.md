...
- gles/web: honor `ColorSpace::Linear` at present time. WebGL's canvas is
  not an sRGB framebuffer, so a linear offscreen blit looked dark. The
  backend now encodes on the present copy; render shaders still output
  linear.
