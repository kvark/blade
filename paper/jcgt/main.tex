...
\paragraph{Host cost.}
Median of the five $256\times256$ samples, nanoseconds:
...
\toprule
 & record & submit & sum \\
\midrule
Blade & $2.7\times 10^{4}$ & $4.92\times 10^{6}$ & $4.95$\,ms \\
wgpu-core & $7.53\times 10^{5}$ & $6.67\times 10^{5}$ & $1.42$\,ms \\
\bottomrule
\end{tabular}
\end{center}

Blade's \texttt{finish} is almost empty, as Table~\ref{tab:split}
predicts; the submit column carries the encoder. wgpu-core's cost sits
in \texttt{finish}. The \emph{sum} on this cell is higher for Blade in
the pilot. That contradicts a naive transfer of the archival sentence
``wgpu's record-and-submit cost is higher in every measured cell'',
which was about the matched synthetic programs, not about a custom
backend that still translates bind groups and still does Blade's
end-of-encoder barrier, running a 30-pass engine graph. Possible
contributors, none isolated here: extra begins on wgpu-core, Blade
submit including the final encoder barrier, unlocked clocks, bind-group
translation paid earlier but perturbing what gets submitted, the
\texttt{Send} wrapper around a non-thread-safe Blade encoder while Bevy
finishes encoders on a task pool.
...
from the unlocked-clock five-sample Bevy pilot as a device-span result.
